...
interface ConfirmSpamModalProps {
  participantNames: string[];
  onConfirm: () => void;
  onCancel: () => void;
}

export function ConfirmSpamModal({ participantNames, onConfirm, onCancel }: ConfirmSpamModalProps) {
  useEffect(() => {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60" onClick={onCancel}>
      ...
      >
        <h2 className="text-base font-semibold text-fg-strong">Mark as spam</h2>
        <p className="mt-2 text-sm text-fg-secondary">
          Mark your conversation with <span className="font-medium text-fg-strong">{name}</span> as spam?
        </p>
        <div className="mt-5 flex justify-end gap-2">
          ...
          >
            Spam <kbd className="ml-1 rounded border border-red-500 px-1 py-px font-mono text-[10px] text-red-200">enter</kbd>
          </button>
        </div>
      </div>
    </div>
