  ...
  if (loading && !post) {
    return (
      <div className={`overflow-hidden rounded-lg border ${
        isMe ? 'border-blue-500/20 bg-blue-500/10' : 'border-ring-muted bg-surface-raised'
      }`}>
        <div className="flex min-h-[6rem] items-center gap-2 px-3 py-3">
          <div className={`h-3 w-3 animate-spin rounded-full border-2 border-t-transparent ${
            isMe ? 'border-fg-muted' : 'border-fg-muted'
          }`} />
          <span className={`text-xs ${isMe ? 'text-fg-secondary' : 'text-fg-muted'}`}>Loading post...</span>
        </div>
      </div>
    );
  }
  ...
  if (post && (post.text || post.authorName)) {
    return (
      ...
      >
        ...
        <div className={`flex items-center gap-2 px-3 py-2 ${
          ...
          {post.authorPicture ? (
            ...
            <div className={`flex h-6 w-6 items-center justify-center rounded-full text-[10px] font-medium ${
              isMe ? 'bg-blue-500/25 text-fg-secondary' : 'bg-surface-muted text-fg-secondary'
            }`}>
              {post.authorName.charAt(0).toUpperCase()}
            </div>
          )}
          <div className="min-w-0 flex-1">
            <p className={`truncate text-xs font-medium ${isMe ? 'text-fg-strong' : 'text-fg-strong'}`}>
              {post.authorName}
            </p>
            {post.authorHeadline && (
              <p className={`truncate text-[10px] leading-tight ${isMe ? 'text-fg-muted' : 'text-fg-muted'}`}>
                {post.authorHeadline}
              </p>
            )}
          </div>
          <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" className={`shrink-0 ${isMe ? 'text-fg-muted' : 'text-fg-faint'}`}>
        ...
        {/* Post text */}
        {post.text && (
          <div className={`px-3 py-2 text-xs leading-relaxed ${isMe ? 'text-fg' : 'text-fg'}`}>
    ...
    >
      <div className={`flex items-center justify-between px-3 py-1.5 ${
        ...
        <div className="flex items-center gap-1.5">
          <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" className={isMe ? 'text-fg-secondary' : 'text-fg-muted'}>
            <path d="M10 13a5 5 0 0 0 7.54.54l3-3a5 5 0 0 0-7.07-7.07l-1.72 1.71" />
            <path d="M14 11a5 5 0 0 0-7.54-.54l-3 3a5 5 0 0 0 7.07 7.07l1.71-1.71" />
          </svg>
          <span className={`text-xs font-medium ${isMe ? 'text-fg-strong' : 'text-fg-secondary'}`}>Shared post</span>
        </div>
        <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" className={`shrink-0 ${isMe ? 'text-fg-muted' : 'text-fg-faint'}`}>
          <path d="M18 13v6a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2V8a2 2 0 0 1 2-2h6" />
          <polyline points="15 3 21 3 21 9" />
